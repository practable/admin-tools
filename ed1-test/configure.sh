...
export SECRETS=~/secret/test.practable.io/ed0
export EXPT_SECRETS=~/secret
export MAIL_SECRET=~/secret/zoho/mail.yaml

export PROJECT=test-practable-io-alpha

# SSH access
export ZONE=europe-west2-a
export INSTANCE=test-practable-io-alpha-ed1
...
export ANSIBLE_GROUP=test_practable_ed1
...
# create ssh.sh
envsubst '${INSTANCE} ${PROJECT} ${ZONE}' < ./templates/ssh.sh.template > ./ssh.sh
chmod +x ./ssh.sh

# create loadtest.sh
envsubst '${INSTANCE} ${PROJECT} ${ZONE}' < ./templates/loadtest-rules.sh.template > ./autogenerated/loadtest-rules.sh
chmod +x ./autogenerated/loadtest-rules.sh
