...
# steps:
# add new cert here, apply
# add new cert to load balancer, apply
# remove old cert from load balancer, apply
# remove old cert from here, apply
...
resource "google_compute_instance_group" "ed-log-dev" {
...
module "gce-lb-http" {
  ...
  ssl_certificates = [google_compute_ssl_certificate.certificate-3.self_link]
  ...
  backends = {
    ...
    ed-log-dev = {
      ...
      groups = [
        {
          group = google_compute_instance_group.ed-log-dev.id
        }
      ]
      iap_config = {
        enable = false
