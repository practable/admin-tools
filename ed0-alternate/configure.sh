...
export INSTANCE=app-practable-io-alpha-ed0-alternate
...
export ANSIBLE_GROUP=app_practable_ed0_alternate
...
# Note that the webhook git-pull-static script has these dirs hard-coded, and needs its template
# editing if these change.
...
# create monitor.service by adding variables to template
export RELAY_AUDIENCE="${HTTPS_HOST}/access"
export RELAY_SECRET=$(cat ${SECRETS}/relay.pat)
envsubst < ./templates/monitor.service.template > ./autogenerated/monitor.service

envsubst < ./templates/relay-monitor-resources.service.template > ./autogenerated/relay-monitor-resources.service
envsubst < ./templates/relay-monitor-server.service.template > ./autogenerated/relay-monitor-server.service
envsubst < ./templates/relay-monitor-server-test.sh.template > ./autogenerated/relay-monitor-server-test.sh
...
envsubst '${ANSIBLE_GROUP}' < ./templates/playbook-disable-monitor.yml.template > ./playbooks/disable-monitor.yml
...
envsubst '${ANSIBLE_GROUP}' < ./templates/playbook-install-relay-monitor.yml.template > ./playbooks/install-relay-monitor.yml
...
envsubst '${DOMAIN}  ${INSTANCE_PATH} ${STATIC_REPO_NAME} ${STATIC_REPO_URL} ${DAVID_STATIC_SUB_DIRS}' < ./templates/webhook-git-pull-static.sh.template > ./autogenerated/git-pull-static.sh
...
envsubst '${HTTPS_HOST}  ${SECRETS}' < ./templates/relay-monitor.sh.template > ./relay/monitor.sh
chmod +x ./relay/monitor.sh
