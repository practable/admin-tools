...
export GRAFANA_EMAIL_FROM=$(yq -r '.username' $MAIL_SECRET)
export GRAFANA_EMAIL_HOST=$(yq -r '.host' $MAIL_SECRET):$(yq -r '.port' $MAIL_SECRET)
export GRAFANA_EMAIL_PASSWORD=$(yq -r '.password' $MAIL_SECRET)
...
cp $SECRETS/htpasswd ./autogenerated/htpasswd

# Create a vars file for ansible, so we can refer to the domain as needed.
envsubst '${DOMAIN}  ${INSTANCE_PATH} ${PROMETHEUS_DATA_DEVICE} ${PROMETHEUS_DATA_MOUNTPOINT} ${PROMETHEUS_VERSION}' < ./templates/vars.yml.template > ./autogenerated/vars.yml
...
envsubst '${ANSIBLE_GROUP}' < ./templates/playbook-update-prometheus-config.yml.template > ./playbooks/update-prometheus-config.yml
envsubst '${ANSIBLE_GROUP}' < ./templates/playbook-update-grafana-config.yml.template > ./playbooks/update-grafana-config.yml
...
envsubst '${GRAFANA_ADMIN_USER} ${GRAFANA_ADMIN_PASSWORD} ${GRAFANA_DOMAIN} ${GRAFANA_ROOT_URL} \
          ${GRAFANA_SERVE_FROM_SUB_PATH} ${GRAFANA_PORT} $GRAFANA_EMAIL_FROM $GRAFANA_EMAIL_HOST \
		  $GRAFANA_EMAIL_PASSWORD' \
  < ./templates/grafana.override.conf.template > ./autogenerated/grafana.override.conf
envsubst '${PROMETHEUS_SCRAPE_INTERVAL}' < ./templates/prometheus.yml.template > ./autogenerated/prometheus.yml
